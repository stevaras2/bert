...
def create_sentences_list():
    ...
    sentences_dataset = pd.read_csv('train_sentences2.csv')
    sentences_list = ['first sentence', 'last sentence']
    for row in sentences_dataset.iterrows():

        sentence = str(row[1][0])
        if sentence not in sentences_list:
            sentences_list.append(sentence)

        previous = str(row[1][1])
        if previous not in sentences_list:
            sentences_list.append(previous)

        next_sen = str(row[1][2])
        if next_sen not in sentences_list:
            sentences_list.append(next_sen)

        section = str(row[1][3])
        if section not in sentences_list:
            sentences_list.append(section)

    with open('fudan_sentences_list.txt','w') as f:
